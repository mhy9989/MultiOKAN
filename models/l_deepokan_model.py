...
class L_DeepOKan_Model_DON_multi(nn.Module):
    def __init__(self, features, p, latent_dim, branch_layers, trunk_layers,tf=True, mf=True, norm = [True,False], 
                branch_type="gram",trunk_type="gram", branch_actfun="silu", trunk_actfun="silu", kan_config={}, **kwargs):
        super(L_DeepOKan_Model_DON_multi, self).__init__()
        self.m = latent_dim
        self.features = features
        self.tf=tf
        self.branch = Branch_multi_KAN(features, p, latent_dim, branch_layers,  tf, mf,
                                        branch_type, norm[0], act_list[branch_actfun.lower()], 
                                        kan_config.copy())
    ...
    def forward(self, x0, x1, **kwargs):
        ...
        y_branch_mul = torch.prod(y_branch, dim=1) if self.tf else y_branch[:,:,0]   # (B, features, latent_dim, p)
...
class L_DeepOKan_Model_DON_Conv_multi(nn.Module):
    def __init__(self, features, p, latent_dim, trunk_layers, branch_channels,
                 kernel_size=3, norm=["Batch",False], tf=True, 
        ...
        self.branch = Branch_ConvKAN_multi(features, p, latent_dim, conv_type, layer_type, 
                                           branch_channels, kernel_size, norm[0], tf,
    ...
    def forward(self, x0, x1, **kwargs):
        #x0:  # (B, 1, features, latent_dim)
        #x1:  # (nt, 1)
        y_branch = self.branch(x0) # (B, features, features/1, latent_dim, p)
        y_trunk = self.trunk(x1)   # (nt, latent_dim, p)
        y_branch_mul = torch.prod(y_branch, dim=1) if self.tf else y_branch[:,:,0] # (B, features, latent_dim, p)
        Y = torch.einsum('ijkl,pkl->ipjk', y_branch_mul, y_trunk) # (B, nt, features, latent_dim)
        return Y