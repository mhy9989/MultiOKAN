...
def SSIM(pred, label):
    ...
    for b in range(label.shape[0]):
        for t in range(label.shape[1]):
            for f in range(label.shape[2]):
                ssim.append(cal_ssim(pred[b, t, f], label[b, t, f], data_range = label[b, t, f].max() - label[b, t, f].min()))
    return np.mean(ssim)


def metric(pred, label, scaler = None, metrics=['mae', 'mse', 'mre'],
           channel_names=None, return_log=True, mode = None):
    ...
    if scaler :
        ...
        for b in range(B):
            for t in range(T):
                for f in range(F):
                    pred[b, t, f] = scaler[f].inverse_transform(pred[b, t, f].reshape(-1,1)).reshape(*pred.shape[3:])
                    label[b, t, f] = scaler[f].inverse_transform(label[b, t, f].reshape(-1,1)).reshape(*label.shape[3:])
    ...
    if 'ssim' in metrics:
        if channel_names is None:
            ...
            eval_res['ssim'] = {}
            for i, c_name in enumerate(channel_names):
                eval_res['ssim'][f'ssim_{str(c_name)}'] = SSIM(pred[:, :, None, i], label[:, :, None, i])
        ...
        if eval_res_type == 0:
            ...
            for k, v in eval_res.items():
                ...
                for vk, vv in v.items():
                    eval_str = f"{vk:<10}:{vv:>12.5e}" if len(eval_log) == 0 else f", {vk:<6}:{vv:>12.5e}"
                    eval_log += eval_str
