...
import numpy as np
#TODO  Branch_ConvKAN_multi_bridge
...
class Branch_ConvKAN_multi(nn.Module):
    ...
    def __init__(self, features:int, p:int, latent_dim:int, conv_type:str, layer_type:str,
                 branch_channels:list, kernel_size, norm, tf=True, actfun=nn.SiLU(), kan_config={}):
        ...
        for _ in range(features):
            self.branch_multi.append(Branch_ConvKAN(features, p, latent_dim, conv_type, layer_type,
                                                    branch_channels, kernel_size, norm, tf, actfun, 
                                                    kan_config))
    
    def forward(self, x): # (B, 1, features, latent_dim)
        B = x.size(0)
        output = torch.zeros(B, self.features, self.f_dim, 
                             self.latent_dim, self.p, device=x.device)
        for i in range(self.features):
            output[:, i] = self.branch_multi[i](x[:, 0, i])
        return output    # (B, features, features/1, latent_dim, p)
...
class Branch_ConvKAN(nn.Module):
    ...
    def __init__(self, features:int, p:int, latent_dim:int, conv_type:str, layer_type:str,
                 channels:list, kernel_size=3, norm="Batch", tf=True, act_fun=nn.SiLU(), kan_config={}):
        ...
        if layer_type=="linear":
            ...
            self.branch.append(nn.Linear(channels[-1] * latent_dim, self.f_dim * p * latent_dim))
        else:
            kan_config["init_l"] = kan_config.get("init_cl", "xv")
            self.branch.append(get_kan(layer_type, 
                                        [channels[-1] * latent_dim, self.f_dim * p * latent_dim],  
                                        act_fun, kan_config))

        self.branch.append(nn.Unflatten(1, (self.f_dim, latent_dim ,p)))

    def forward(self, x): # (B, latent_dim)
        x = self.branch(x)
        return x    # (B, features/1, latent_dim, p)
