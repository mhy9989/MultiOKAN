# Based 0n this: https://github.com/IvanDrokin/torch-conv-kan/blob/main/kan_convs/kan_conv.py
import torch
...
class KANConvNDLayer(nn.Module):
    def __init__(self, conv_class, input_dim, output_dim, spline_order, kernel_size,
                 ...
                 ndim: int = 2, grid_size=5, base_activation=nn.SiLU(), grid_range=[-1, 1], dropout=0.0,
                 if_tanh=False,**norm_kwargs):
    ...
    def forward_kan(self, x, group_index):
        ...
        x = base_output + spline_output #self.prelus[group_index](self.layer_norm[group_index](base_output + spline_output))
...
class KANConv3DLayer(KANConvNDLayer):
    def __init__(self, input_dim, output_dim, kernel_size, spline_order=3, groups=1, padding=0, stride=1, dilation=1,
                 grid_size=5, base_activation=nn.SiLU(), grid_range=[-1, 1], dropout=0.0,
                 **norm_kwargs):
        super(KANConv3DLayer, self).__init__(nn.Conv3d,
...
class KANConv2DLayer(KANConvNDLayer):
    def __init__(self, input_dim, output_dim, kernel_size, stride=1, padding=0, dilation=1, grid_size=5, spline_order=3,
                 groups=1, base_activation=nn.SiLU(), grid_range=[-1, 1], dropout=0.0,if_tanh=False,
                 **norm_kwargs):
        super(KANConv2DLayer, self).__init__(nn.Conv2d,
                                             input_dim, output_dim,
                                             spline_order, kernel_size,
                                             groups=groups, padding=padding, stride=stride, dilation=dilation,
                                             ndim=2,
                                             grid_size=grid_size, base_activation=base_activation,
                                             grid_range=grid_range, dropout=dropout, if_tanh=if_tanh,
                                             **norm_kwargs)
...
class KANConv1DLayer(KANConvNDLayer):
    def __init__(self, input_dim, output_dim, kernel_size, spline_order=3, groups=1, padding=0, stride=1, dilation=1,
                 grid_size=5, base_activation=nn.SiLU(), grid_range=[-1, 1], dropout=0.0,
                 **norm_kwargs):
        super(KANConv1DLayer, self).__init__(nn.Conv1d,
                                             input_dim, output_dim,
                                             spline_order, kernel_size,
                                             groups=groups, padding=padding, stride=stride, dilation=dilation,
                                             ndim=1,
...
def sampling_generator_kan(N, reverse=False, mid_conv=True):
